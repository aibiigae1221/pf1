...
//영역에서 벗어나면(마우스가 떠나면) 서브메뉴가 닫히게 처리
    ...
    // 학력 및 학위 / 연보 / 수상 및 수훈 탭 클릭
   
    $(".navtab li a").click(function(e){
        e.preventDefault();
        $(".navtab li.current a").removeClass("blue");
        $(".navtab li.current").removeClass("current");
        ...
        $(`.nav-list:nth-child(${(idx+1)})`).removeClass("hide");
    });

    $(".navtab li:first-child a").click();
